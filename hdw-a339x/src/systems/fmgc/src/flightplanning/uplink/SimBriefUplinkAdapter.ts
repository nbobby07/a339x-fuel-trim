...
import { isCruiseStepInsertionValid } from '@fmgc/flightplanning/CruiseStep';
...
export interface SimBriefOfpWithCruiseSteps extends ISimbriefData {
...
export class SimBriefUplinkAdapter {
  ...
  static async uplinkFlightPlanFromSimbrief<P extends FlightPlanPerformanceData>(
    ...
    intoPlan: number,
    ofp: SimBriefOfpWithCruiseSteps,
    ...
    await ensureAirwaysFinalized();
    await this.uplinkCruiseSteps(fms, flightPlanService, ofp);
    fms.onUplinkDone(true, intoPlan);
  }

  /** Import explicit SimBrief steps after airway expansion, using the same cruise-step model as STEP ALTS. */
  static async uplinkCruiseSteps<P extends FlightPlanPerformanceData>(
    fms: FmsDisplayInterface,
    flightPlanService: FlightPlanService<P>,
    ofp: SimBriefOfpWithCruiseSteps,
  ) {
    if (!ofp.stepClimbString?.trim()) {
      return;
    }
    if (ofp.stepClimbString.length > 8192) {
      SimBriefUplinkAdapter.logTroubleshootingError(
        fms,
        '[SimBriefUplinkAdapter] Step metadata too long. Review STEP ALTS.',
      );
      return;
    }
    const plan = flightPlanService.uplink;
    const steps: { waypointIndex: number; toAltitude: number }[] = [];
    let afterIndex = Math.max(0, plan.activeLegIndex - 1);
    const report = (token: string, reason: string) =>
      SimBriefUplinkAdapter.logTroubleshootingError(
        fms,
        `[SimBriefUplinkAdapter] Step ${token} not imported: ${reason}. Review STEP ALTS.`,
      );
    const entries: { ident: string; level: string }[] = [];
    // SimBrief serializes a continuous IDENT/FL/IDENT/FL chain. Also accept separated pairs.
    // A slash inside an ident is ambiguous in this format and must not shift the remaining pairs.
    for (const chain of ofp.stepClimbString.trim().toUpperCase().split(/\s+/)) {
      const fields = chain.split('/');
      if (
        fields.length < 2 ||
        fields.length % 2 !== 0 ||
        fields.some((field, index) => !(index % 2 ? /^\d{2,4}$/ : /^[A-Z0-9]+$/).test(field))
      ) {
        report(chain, 'invalid or ambiguous format');
        continue;
      }
      for (let index = 0; index < fields.length; index += 2) {
        entries.push({ ident: fields[index], level: fields[index + 1] });
      }
    }
    for (const { ident, level } of entries) {
      const token = `${ident}/${level}`;
      const altitude = Number(level) * 100;
      if (ident === ofp.origin.icao && altitude === Number(ofp.cruiseAltitude)) {
        continue; // The first entry is the initial cruise level, not a step.
      }
      if (steps.length >= 4) {
        report(token, 'four-step limit');
        continue;
      }
      const hints = (ofp.navlog ?? []).filter(
        (fix) => fix.ident === ident && Number.isFinite(Number(fix.pos_lat)) && Number.isFinite(Number(fix.pos_long)),
      );
      const candidates = plan.allLegs
        .map((leg, index) => ({ leg, index }))
        .filter(({ leg, index }) => {
          if (index <= afterIndex || index >= plan.firstMissedApproachLegIndex || leg.isDiscontinuity === true) {
            return false;
          }
          const waypoint = leg.terminationWaypoint();
          if (!waypoint) {
            return false;
          }
          // Lat/long waypoints get a generated FMC ident, so use the OFP coordinate there.
          return (
            (waypoint.ident === ident &&
              (hints.length === 0 ||
                hints.some(
                  (fix) => distanceTo(waypoint.location, { lat: Number(fix.pos_lat), long: Number(fix.pos_long) }) < 1,
                ))) ||
            hints.some(
              (fix) =>
                fix.type === 'ltlg' &&
                distanceTo(waypoint.location, { lat: Number(fix.pos_lat), long: Number(fix.pos_long) }) < 1,
            )
          );
        });
      if (candidates.length !== 1) {
        report(token, candidates.length ? 'ambiguous waypoint' : 'waypoint not found');
        continue;
      }
      const index = candidates[0].index;
      if (!isCruiseStepInsertionValid(steps, index, altitude, Number(ofp.cruiseAltitude) / 100)) {
        report(token, 'altitude or step sequence outside supported limits');
        continue;
      }
      await flightPlanService.addOrUpdateCruiseStep(index, altitude, FlightPlanIndex.Uplink);
      steps.push({ waypointIndex: index, toAltitude: altitude });
      afterIndex = index;
    }
  }

  static async downloadOfpForUserID(username?: string, userID?: string): Promise<SimBriefOfpWithCruiseSteps> {
    ...
    try {
      ...
      return {
        ...simbriefDataParser(body),
        stepClimbString: typeof body.general?.stepclimb_string === 'string' ? body.general.stepclimb_string : undefined,
      };
    } catch (e) {
