...
export class CruisePathBuilder {
  ...
  computeCruisePath(
    ...
    if (speedTarget - sequence.lastCheckpoint.speed > 1) {
      ...
      assertValidForwardStep(accelerationStep);
  ...
  private tryAddStepFromLastCheckpoint(
    ...
    try {
      ...
      assertValidForwardStep(stepResults);
      if (!Number.isFinite(step.toAltitude) || Math.abs(stepResults.finalAltitude - step.toAltitude) > 1e-6) {
        return false;
      }
      if (
        (isClimbVsDescent && stepResults.finalAltitude <= altitude) ||
        (!isClimbVsDescent && step.toAltitude < altitude && stepResults.finalAltitude >= altitude)
      ) {
        return false;
      }
    } catch (error) {
      if (error instanceof InvalidVnavPredictionError) {
        return false;
      }
      throw error;
    }
  ...
  private computeCruiseSegment(
    ...
    if (!Number.isFinite(targetDistanceFromStart)) {
      throw new InvalidVnavPredictionError('Non-finite cruise target distance');
    }
    ...
    for (; numIterations < 1000 && state.distanceFromStart < targetDistanceFromStart; numIterations++) {
      ...
      if (step.distanceTraveled <= 0) {
        throw new InvalidVnavPredictionError('Cruise prediction makes no distance progress');
      }
    ...
    if (state.distanceFromStart < targetDistanceFromStart) {
      throw new InvalidVnavPredictionError('Cruise prediction did not reach the target distance');
    }
