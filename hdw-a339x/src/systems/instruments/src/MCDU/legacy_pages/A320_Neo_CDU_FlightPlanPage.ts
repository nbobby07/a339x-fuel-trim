...
import { plannedCruiseLevelAtWaypoint } from '@fmgc/flightplanning/CruiseStep';
  ...
  static createScrollWindow(
    ...
    for (let rowI = 0, winI = offset; rowI < rowsCount; rowI++, winI++) {
      ...
      if (wp && wp.isDiscontinuity === false) {
        ...
        // On the ground, en-route rows show the planned cruise schedule, not an interpolated climb.
        ...
        const plannedCruiseLevel =
          ...
          !inMissedApproach &&
          !hasAltConstraint &&
          wp.segment === targetPlan.enrouteSegment &&
          wp.type !== LegType.HM
            ? plannedCruiseLevelAtWaypoint(
                targetPlan.performanceData.cruiseFlightLevel.get(),
                targetPlan.allLegs.flatMap((leg, index) =>
                  leg.isDiscontinuity === false && leg.cruiseStep
                    ? [{ waypointIndex: index, toAltitude: leg.cruiseStep.toAltitude }]
                    : [],
                ),
                fpIndex,
              )
        ...
        if (targetPlan.index !== FlightPlanIndex.Temporary) {
          ...
          } else if (plannedCruiseLevel !== undefined) {
            altitudeConstraint = `\xa0FL${plannedCruiseLevel.toFixed(0).padStart(3, '0')}`;
            altColor = color;
            altSize = 'small';
