...
pub struct FuelTank {
    ...
    density_id: VariableIdentifier,
    density_kg_per_gallon: f64,
...
impl FuelTank {
    pub fn new(context: &mut InitContext, id: &str, location: Vector3<f64>, write: bool) -> Self {
        FuelTank {
            ...
            density_id: context.get_identifier("FUEL WEIGHT PER GALLON".to_owned()),
            // Startup fallback for hosts/tests that have not provided native density.
            density_kg_per_gallon: FUEL_GALLONS_TO_KG,
            location,
    ...
    pub fn volume_gallons(&self) -> f64 {
        self.quantity.get::<kilogram>() / self.density_kg_per_gallon
    }
...
impl SimulationElement for FuelTank {
    fn read(&mut self, reader: &mut SimulatorReader) {
        ...
        let native_density: f64 = reader.read(&self.density_id);
        // Retain the last valid density if a subsequent read is unavailable.
        let density = if native_density.is_finite() && native_density > 0. {
            native_density
        } else {
            self.density_kg_per_gallon
        };
        let mass = volume * density;
        if volume.is_finite() && volume >= 0. && mass.is_finite() {
            self.density_kg_per_gallon = density;
            self.quantity = Mass::new::<kilogram>(mass);
        }
    }
